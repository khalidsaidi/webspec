...
export type IntentSpec = {
  feature: { id: string; title: string; summary?: string };
  target: TargetKernel;
  intent: {
    pages: IntentPage[];
  };
...
export type PlanStep =
  ...
  | { kind: "insertBetweenMarkers"; path: string; markerStart: string; markerEnd: string; content: string; uniqueKey?: string }
  | { kind: "assertFileExists"; path: string }
  | { kind: "assertFileContains"; path: string; needle: string };
...
export type RiskAssessment = {
  ...
  label: RiskLabel;
  action: RiskAction;
  reasons: Array<{ title: string; evidence: string; suggestion: string; strength: number }>;
};
...
export type Proposal = {
  ...
  intent: IntentSpec;
  assumptions: string[];
  compile: { ok: boolean; diagnostics: Diagnostic[]; plan?: Plan };
  risk?: RiskAssessment;
};
