import type { Plan, RiskAssessment, RiskLabel, RiskAction } from "./types.js";
...
function centroid(agg: MF, samples = 401): number {
  let num = 0;
  let den = 0;
...
export function assessRisk(plan: Plan, profile: "fast" | "balanced" | "strict" = "balanced"): RiskAssessment {
  ...
  const rules: Array<{ name: string; fire: number; out: MF; evidence: string; suggestion: string }> = [];
  ...
  if (score >= 0.80) label = "EXTREME";
  else if (score >= 0.62) label = "HIGH";
  else if (score >= 0.40) label = "MEDIUM";
