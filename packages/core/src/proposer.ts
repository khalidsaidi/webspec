...
function wantsAvoidDeps(c?: GoalConstraints) {
  return (c?.newDependencies ?? "avoid") === "avoid";
}

type RecipeKind = "playground" | "feedback" | "settings";

function classifyGoal(text: string): RecipeKind {
  const t = text.toLowerCase();
  if (t.includes("feedback") || t.includes("contact") || t.includes("report a bug")) return "feedback";
  if (t.includes("setting") || t.includes("preferences") || t.includes("profile")) return "settings";
  return "playground";
}
