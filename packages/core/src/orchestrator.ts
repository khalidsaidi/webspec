...
export type GenerateResponse = {
  ok: boolean;
  patch: string;
  verification: { ok: boolean; diagnostics: Diagnostic[] };
...
export async function propose(req: ProposeRequest): Promise<ProposeResponse> {
  ...
  const wsReader = req.workspace?.reader ?? {
    ...
    async exists(p: string) { return Object.prototype.hasOwnProperty.call(SANDBOX_SNAPSHOT, p); },
    async read(p: string) {
      const v = SANDBOX_SNAPSHOT[p];
      if (v === undefined) throw new Error(`Missing demo file: ${p}`);
      return v;
    }
  };
...
export async function generatePatch(req: GenerateRequest): Promise<GenerateResponse> {
...
export async function applyPlanToDisk(ws: WorkspaceWriter, plan: Plan): Promise<{ ok: boolean; diagnostics: Diagnostic[] }> {
  const diagnostics: Diagnostic[] = [];
  ...
  for (const step of plan.steps) {
    ...
    if (step.kind === "insertBetweenMarkers") {
      ...
      if (step.uniqueKey && old.includes(step.uniqueKey)) continue;
  ...
  for (const p of touched) {
    if (await ws.exists(p)) after[p] = await ws.read(p);
  }
  const verification = verifyAfterState(after, plan);
